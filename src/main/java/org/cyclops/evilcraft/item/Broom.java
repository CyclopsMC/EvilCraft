...
public class Broom extends ConfigurableDamageIndicatedItemFluidContainer implements IBroom {
    ...
    public Broom(ExtendedConfig<ItemConfig> eConfig) {
        super(eConfig, 10 * Fluid.BUCKET_VOLUME, Blood.getInstance());
        this.maxStackSize = 1;
    ...
    public void onRenderOverlayEvent(RenderGameOverlayEvent.Post event) {
        ...
        if (player.getRidingEntity() instanceof EntityBroom
            ...
            RenderOverlayEventHook.OverlayPosition overlayPosition = RenderOverlayEventHook.OverlayPosition.values()[
                    MathHelper.clamp(BroomConfig.guiOverlayPosition, 0, RenderOverlayEventHook.OverlayPosition.values().length - 1)];
