...
public class ItemVengeanceRing extends Item {
    ...
    public void inventoryTick(ItemStack itemStack, ServerLevel level, Entity entity, @Nullable EquipmentSlot slot) {
        if(entity instanceof Player && !level.isClientSide()
                && IModHelpers.get().getWorldHelpers().efficientTick(level, BONUS_TICK_MODULUS, entity.getId())) {
            int area = ItemVengeanceRingConfig.areaOfEffect;
            toggleVengeanceArea(level, entity, area, ItemHelpers.isActivated(itemStack), true, false);
