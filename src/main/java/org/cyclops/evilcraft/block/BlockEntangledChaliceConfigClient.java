...
public class BlockEntangledChaliceConfigClient extends BlockClientConfig<ModBaseNeoForge<?>> {
    public BlockEntangledChaliceConfigClient(BlockConfigCommon<ModBaseNeoForge<?>> blockConfig) {
        super(blockConfig);
        blockConfig.getMod().getModEventBus().addListener((RegisterSpecialModelRendererEvent event) -> event.register(Identifier.fromNamespaceAndPath(Reference.MOD_ID, "entangled_chalice_fluid_overlay"), RenderItemStackBlockEntityEntangledChalice.Unbaked.MAP_CODEC));
