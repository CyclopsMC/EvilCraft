...
public class DarkTempleGenerator implements IWorldGenerator {
	...
	public static DarkTempleData getCachedData(World world) {
		return darkTemples.get(world.provider.getDimensionId());
	}

	private DarkTempleData loadData(World world) {
		DarkTempleData data = (DarkTempleData) world.getPerWorldStorage().loadData(DarkTempleData.class, DARK_TEMPLE_MAP_NAME);
		if(data == null) {
			data = new DarkTempleData(DARK_TEMPLE_MAP_NAME);
			world.getPerWorldStorage().setData(DARK_TEMPLE_MAP_NAME, data);
		}
		return data;
	}
